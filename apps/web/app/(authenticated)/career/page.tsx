  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-zinc-800">Career Tracks</h1>
          ...
          return (
            <Card key={track.id}>
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h2 className="font-semibold text-zinc-800">{track.label}</h2>
                  <p className="mt-1 text-sm text-zinc-500">{track.description}</p>
                </div>
                <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${active ? 'bg-green-100 text-green-700' : 'bg-zinc-100 text-zinc-500'}`}>
        ...
        {readiness.map((role) => (
          <Card key={role.roleTrackId}>
            <div className="mb-4 flex items-center justify-between gap-4">
              <div>
                <h2 className="text-lg font-semibold text-zinc-800">{role.roleTrack.label}</h2>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              {role.areas.map((area) => (
                <div key={area.area} className="rounded-lg border border-zinc-200 p-3">
                  <div className="mb-2 flex items-center justify-between">
                    <span className="text-sm font-medium text-zinc-700">{area.area.replaceAll('_', ' ')}</span>
              ...
              <div className="mt-5 border-t border-zinc-100 pt-4">
                ...
                <div className="space-y-2">
                  {role.topGaps.slice(0, 4).map((gap) => (
                    <div key={gap.id} className="text-sm text-zinc-600">
                      <span className="font-medium text-zinc-800">{gap.title}</span> - {gap.description}
                    </div>
                  ))}
                </div>
              </div>
