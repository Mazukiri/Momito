  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-zinc-800">Learning Ledger</h1>
        ...
        <button onClick={() => router.push('/learning/inbox')} className="rounded-lg border border-zinc-300 px-4 py-2 text-sm font-medium text-zinc-700">
          Reading Inbox
        </button>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[340px_1fr]">
        <aside className="space-y-6">
          <Card>
            <h2 className="mb-3 font-semibold text-zinc-800">Readwise</h2>
            <p className="text-sm text-zinc-500">
              Status: <span className="font-medium text-zinc-700">{connection?.status ?? 'not connected'}</span>
            </p>
            ...
            <button onClick={sync} disabled={working || !connection?.hasToken} className="mt-3 w-full rounded-lg border border-zinc-300 px-4 py-2 text-sm font-medium text-zinc-700 disabled:opacity-50">
              Sync Now
            </button>
          </Card>

          <Card>
            <h2 className="mb-3 font-semibold text-zinc-800">Add Evidence</h2>
            <form onSubmit={addEvidence} className="space-y-3">
              <input value={title} onChange={(event) => setTitle(event.target.value)} required placeholder="What did you learn?" className="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm" />
              <select value={roleTrackId} onChange={(event) => setRoleTrackId(event.target.value)} className="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm">
                {Object.values(CAREER_ROLE_TRACKS).map((track) => <option key={track.id} value={track.id}>{track.label}</option>)}
              </select>
              <select value={area} onChange={(event) => setArea(event.target.value)} className="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm">
                {CAREER_ROLE_AREA_IDS.map((item) => <option key={item} value={item}>{item.replaceAll('_', ' ')}</option>)}
              </select>
              <textarea value={body} onChange={(event) => setBody(event.target.value)} rows={4} placeholder="Notes" className="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm" />
