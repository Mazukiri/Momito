...
- **MOM-146** Readwise highlight → FSRS spaced-repetition review (the read→retain loop) · **DONE** 2026-07-09 (no schema — `ReviewState.objectType` is a plain string, `objectId` a uuid, so `'highlight'` is purely additive). **Why:** a synced highlight used to dead-end in a write-only ledger after the learner tagged it — the app's whole edge (FSRS retention tied to a target) stopped one step short. Now reviewing a highlight as anything but "ignored" seeds a `ReviewState` via `ReviewsService.record(userId,'highlight',id,3)`, so the insight resurfaces on Today (cued by its source, text revealed on recall) until it sticks. `reviews.service` supports `'highlight'` (ownership-checked like Story; `listDue` titles by source); `learning.service.updateHighlight` seeds the review (best-effort) + `getHighlight` feeds the review card's reveal; a highlight deleted upstream in Readwise suspends its review state on next sync. `TodayReviewCard` gains a highlight branch; inbox "Review" → "Remember (spaced review)". 220 API + 25 web tests (+11/+1); live-verified end-to-end: review useful → ReviewState created → appears in `/reviews/due` titled "Atomic Habits" → suspends on delete.
...
- **MOM-104** Implement timestamp + transition-history migration + service writes · **DONE** 2026-07-10 (additive migration `job_event_transition_status`: `JobEvent.fromStatus`/`toStatus` + title-parse backfill; `jobs.service.update()` writes the pair on each transition; `funnel()` emits `medianDaysInStage` per stage from ordered `status_change` eventAt diffs; funnel card shows `~Nd` timing. 241 API tests; live: backfill + fresh transition + median 1.5/0 verified. ADR-0009 ACCEPTED.) · *migration*
- **MOM-105** Time-in-stage stall detection on job list + Today · **DONE** 2026-07-10 (no schema: `JOB_STAGE_STALL_THRESHOLDS` + `daysInStage`/`isStalled` on `JobApplicationResponse` via a `take:1` latest-status_change include; `recommendations.service` emits a "Follow up on {company} — {N}d in {stage}" card (priority 68) that supersedes the routine stage card; jobs list `⏳ Stalled` badge. 245 API tests; live: backdated saved job flipped isStalled true at 30d≥14.)
- **MOM-106** Rejection-reason + loss-analysis capture (`REJECTION_REASONS` on JobApplication) · **DONE** 2026-07-10 (additive migration `job_rejection_reason`: `rejectionReason String?`; `REJECTION_REASONS` taxonomy; service accepts it only when the resulting status is `rejected` (400 otherwise), clears on reopen, copies onto the transition event's notes; `funnel().byRejectionReason` count breakdown; job-page reason select + funnel "Why rejected" row. 251 API tests; live: 400 guard, store + event note, breakdown, auto-clear all verified.) · *migration*
- **MOM-107** Kanban board pipeline view (drag → existing status `update()`) · **DONE** 2026-07-10 (no schema, web-only: list↔board toggle on jobs page; column per `JOB_FUNNEL_STAGES`; native HTML5 drag → optimistic `moveJob` calling `jobsApi.update(id,{status})` with revert-on-error; funnel + pills stay on list view. 26 web tests; no Playwright bin for a screenshot — drag delegates to the MOM-102/104/106 live-verified status-update path.) **★ Wave 2 (pipeline truth) COMPLETE.**
...
- **MOM-114** SPIKE-015 + structured `Offer` model (base/equity/bonus/location/visa/deadline) · **DONE** 2026-07-10 (migration `offers`: one-per-job `Offer` model, Decimal comp + equityYears, OFFER_STATUSES; `offers` module — `/offers` + per-job `/jobs/:jobId/offer` upsert; `OfferCard` on job detail. ADR-0015 ACCEPTED, D-019, SPIKE-015 resolved. 267 API tests; live: normalized 275k, currency upper-cased, 400 guard, delete.) · *migration*
- **MOM-115** Offer comparison decision view (normalized, visa-adjusted) · **DONE** 2026-07-10 (no schema: `/offers` comparison table sorted by normalized annual total (base+bonus+equity/years, computed in the serializer), visa-sponsored column, single-currency v1 with a mixed-currency warning; `💰 Offers` nav. Live-verified with the MOM-114 endpoint.) **★ Wave 3 (contacts + offers) COMPLETE.**
...
- **MOM-119** Referral network view + Today thank-you nudges · **DONE** 2026-07-10 (no schema, web-only: `/contacts` page grouped by relationship with per-contact linked-job chips; `👥 Contacts` nav entry. Today thank-you nudge is already delivered by the MOM-118 `thank_you` reminder on the Today feed — no redundant rec card. 26 web tests.) **★ Wave 3 contacts (116/117/118/119) COMPLETE.**
...
- **MOM-122** Add `companyId` FK + link/backfill UI (**highest-risk migration**) · **DONE** 2026-07-10 (additive migration `job_company_fk`: nullable `company_id` FK `SetNull`, free-text `company` kept; exact ci name-match backfill in the migration SQL with `HAVING count(*)=1` dup guard, `--create-only` then completed before apply. jobs.service validates+serializes `companyRef`; DTOs `@IsUUID`; web add-form datalist link + detail-page Company link/unlink card. 234 API tests; live: 6/7 backfilled, Datadog stays null, link/unlink/reject correct.) · *migration* · **follow-up:** wire `career.service` readiness to `companyRef.focusAreas`.
- **MOM-123** Company detail page (interview process, focus, sponsorship, linked content) · **DONE** 2026-07-10 (no schema: catalog `companies/page.tsx` + detail `companies/[id]/page.tsx` — focus-area weight bars, sponsorship badge, comp band, numbered interview process, role tracks, linked question/story counts; `🏢 Companies` nav entry; `CompaniesService.get()` includes `_count` questions/stories → `linkedQuestionCount`/`linkedStoryCount` on `CompanyResponse`. 237 API tests; live: Meta → 4 linked questions.)
- **MOM-124** Visa/sponsorship filter + sort on catalog + job list · **DONE** 2026-07-10 (no schema, web-only: sponsorship filter-pill row on the companies catalog and the jobs list — `sponsorshipKey` buckets null with 'unknown'; jobs list also sorts sponsored-first via `jobSponsorship` = `companyRef.sponsorshipStatus ?? visaTag ?? 'unknown'` + `SPONSORSHIP_RANK`; filters compose with the status pills. 26 web tests.)
- **MOM-125** Company targeting/fit shortlist (fit × company-readiness × sponsorship × region) · **DONE** 2026-07-10 (no schema: `career.getTargetShortlist` ranks the catalog by focus-weighted grounded readiness × sponsorship multiplier (sponsored 1.0/unknown 0.7/not_sponsoring 0.2) × region preference (inferred from the user's pipeline; off-target 0.85); readiness memoized per distinct role track; `GET /career/target-shortlist`; "Where to apply next" top-3 card on the career page. 239 API tests; live: NVIDIA 51 → Amazon 15, ByteDance unknown drops to 15 on ×0.7.)
...
- **MOM-131** Story coverage → specific-interview behavioral gap map · **DONE** 2026-07-09 (no schema; `career.getJobStoryGaps` maps the target role's expected STAR competencies — the concrete `STORY_COMPETENCIES` ids named in the track's behavioral-checklist keywords, else the universal ownership/conflict/ambiguity/failure set — against the user's story bank by `competencyTags` (case-insensitive), route `GET /career/jobs/:jobId/story-gaps`, `StoryGapCard` under the readiness card on the job page. 223 API tests; live-verified: Meta → 1/4 covered (ownership), missing conflict/failure/ambiguity. Company-specific weighting enriches when MOM-121's structured Company lands.)

### Track Q — Résumé Versioning, Tailoring & Artifacts · CareerOS Gate 4 *(AI-first; key coming)*
- **MOM-132** SPIKE-014 + DESIGN: `ResumeVersion` decoupled from `@unique Profile` · **DESIGN DONE** 2026-07-10 (ADR-0012 ACCEPTED, D-020; SPIKE-014 resolved: `contentMd` Markdown derived from Profile via `profileToMarkdown`; PDF-lib choice deferred to MOM-139). Implemented with MOM-133.
...
- **MOM-165** Navigation restructure into sections (Momito 1.0 PR 6) · **DONE** 2026-07-12 (no schema). Flat 16-entry sidebar → `NavItem.section` grouping the 11 secondary items into Pipeline / Prep / Insights, Settings standalone. 5 primary items + `BottomTabs` + the ≤5-primary guard unchanged. `Sidebar` renders a section header when the section changes. Web typecheck + lint + 34 tests green.
- **MOM-164** Strip vestigial write-only job fields (Momito 1.0 PR 5) · **DONE** 2026-07-12 (no schema). `referralName` (→ Contacts, MOM-116), `h1bCountLastYear`, `compensationNotes` were accepted/persisted/serialized but had zero web UI. Removed from create/update DTOs, service writes + serializer, and `JobApplicationResponse`/`CreateJobApplicationRequest`. DB columns preserved. 328 API + 34 web tests; live: response drops the fields, a create sending `referralName` → 400 (forbidNonWhitelisted), normal create still works.
- **MOM-163** Retire Missions — API half + shared types (Momito 1.0 PR 4) · **DONE** 2026-07-12 (no schema, D-021). Deleted `apps/api/src/missions/` + spec, unregistered `MissionsModule`, dropped the `MissionsService` couplings from recommendations (priority-110 mission cards + overdue-task deep-link → `/calendar`) and dashboard (focusMission/activeMissions/todayPlanItems), and removed all `Mission*`/`WeeklyPlan*`/`PlanItem*` types + constants from `@momito/shared`. Schema + all 5 mission tables + `missionId` FK columns preserved (restorable via revert). 328 API + 34 web tests green; live: `/missions` 404, zero mission cards on Today, dashboard summary clean.
- **MOM-162** Retire Missions — web half (Momito 1.0 PR 3) · **DONE** 2026-07-12 (no schema, D-021). Deleted the two `missions/` pages, the `Mission` nav entry, `missionsApi` + its 7 type imports, the job page's Open/Create Mission button, the dashboard Focus Mission card, and the `?missionId` filters on calendar/learning + the practice/new pass-through. Web-first (API still serves `/missions` until PR 4, harmless). `missionId` params on tasks/learning/session creation kept (columns stay). Web lint + typecheck + 34 tests green.
- **MOM-161** Grading reaches résumé-AI hardening parity (Momito 1.0 PR 2) · **DONE** 2026-07-12 (no schema). `grading.service.ts` never got MOM-147/148's hardening: still `MAX_TOKENS=4096` (truncates once adaptive thinking bills inside it) and no completeness contract (a cheap model could silently score 3 of 5 criteria). Now `MAX_TOKENS=16000`; rubric criteria enumerated `0..n-1`; `GradeResultSchema.criteriaScores` gains `index`; a first pass that skips criteria triggers one repair pass (merged, both calls billed), a failed repair keeps the partial; no-rubric grading exempt (one call). `formatFeedback()` unchanged. 330 API tests + live: a 4-criterion grade scored all 4 (aiScore 0.41, one request, $0.0165). See LOG.
- **MOM-171** Web test hardening (Momito 1.1 PR 6) · **DONE** 2026-07-12 (no schema). Flaky résumé Accept asserts await the post-click flush; new `JobReadinessCard` tests (resolve/dismiss + repairing tag — promised by MOM-167); new jobs-list tests (sponsorship filter, stalled badge, list→board). 42 web tests (+7). No API change.
- **MOM-170** ATS matching understands lowercase + multi-word skills (Momito 1.1 PR 5) · **DONE** 2026-07-12 (no schema). `extractJdSkills` only caught `[A-Z]`-initial tokens → lowercase and multi-word skills invisible to coverage + the gap→task bridge. Now unions the capitalized pass with a word-boundary phrase scan against a 143-phrase lexicon (from `CAREER_ROLE_TRACKS` keywords) + the user's profile skills; multi-word phrases substring-match the artifact text, single tokens keep exact membership. MOM-159 dot-strip preserved. 343 API tests (+3 counterfactual); live: an all-lowercase JD extracted 3 keywords (typescript covered, 2 phrases missing) where the old pass extracted zero.
- **MOM-169** Readiness computed once per request (Momito 1.1 PR 4) · **DONE** 2026-07-12 (no schema, perf). `getReadiness` ran the heavy per-user `areaMastery` sweep (full ReviewState + attempt scan) once per goal / per track. Now takes an optional `sharedMastery`; `listActiveReadiness` + `getTargetShortlist` compute it once and share it. Dashboard benefits automatically. No behavior change. 340 API tests (+1: areaMastery called once for 3 goals); live: `roleReadiness` byte-identical before/after (2 sweeps → 1 for the demo's 2 goals).
- **MOM-168** AI grade feeds FSRS — one-tap suggested rating (Momito 1.1 PR 3) · **DONE** 2026-07-12 (no schema). `GradeResult.suggestedRating` was computed but only Markdown-embedded; the FSRS rating stayed manual. `gradeAttempt` now returns it structured (null on cached — unrecoverable from Markdown); `AiFeedbackCard` shows a one-tap "Rate '<X>' — the AI's suggestion" (fresh grade + not-yet-rated) that routes through `RevealPanel.rate()`. Map is the inverse of `SELF_RATING_TO_GRADE`: again→1, hard→2, good→3, easy→5. 339 API + 35 web tests; live: fresh grade → `suggestedRating:'again'`, cached → null.
- **MOM-167** Wire the dead resolve/dismiss endpoints + surface repairing (Momito 1.1 PR 2) · **DONE** 2026-07-12 (no schema). `POST /weaknesses/signals/:id/{resolve,dismiss}` + their client methods existed with **zero UI callers**. `JobReadinessCard` signal chips now carry ✓ resolve / ✕ dismiss (optimistic + reload); a `repairing` signal renders amber with a tag. Today's repair card titles `Repairing:` (+ "one more rep" nudge) when repairing. Today dismiss deliberately deferred (the card is a nav button — nested button = invalid HTML; full control lives on the job card). 339 API + 34 web tests; live: resolve endpoint returned `resolved` and dropped the signal.
- **MOM-166** Repair sessions actually repair (Momito 1.1 PR 1) · **DONE** 2026-07-12 (no schema, D-023). The repair loop was half-closed: a bombed debrief emits a `WeaknessSignal` → Today "Repair:" card → the user runs the `weak_area_review`/`job_prep` session → and nothing touched the signal (`repairing` was dead; resolve/dismiss endpoints had no UI). Now completing such a session credits its **positive attempts** (canonical `isPositiveAttempt`, extracted to a pure exported fn) against OPEN area-signals: 1 positive → `repairing`, 2 (or 1 more when already repairing) → `resolved`; reason-signals stay manual. `sessions.finish()` computes `{area,positive}` and hands it to `WeaknessesService.creditRepairEvidence` (persistence-only); gated to completed sessions, try/warn so it never breaks finish. **Live testing caught a real bug** — the enum value is `weak_area_review`, not `weakness_repair`. 338 API tests + live: debrief→open, 1 positive→repairing, 1 more→resolved. See LOG + D-023.
- **MOM-160** True ever-reached funnel conversion (Momito 1.0 PR 1) · **DONE** 2026-07-12 (no schema, D-022). `funnel()` counted only currently-active apps toward `reached`/conversion/responseRate, so rejected/withdrawn apps vanished from denominators and every conversion rate was inflated. New `maxStageReached()` computes each app's deepest-ever funnel stage from current status ∪ its `status_change` transition history (pre-MOM-102 terminal apps fall back to applied-if-appliedDate else saved); `reached`/conversion/responseRate and the source/visa/résumé breakdown offers+interviewing counts all derive from it. `atStage` stays a snapshot; `active` decoupled to the non-terminal count. `JobFunnelResponse` shape unchanged (no web change). 326 API tests + live: a throwaway walked to interview-then-rejected kept its interview credit (reached 3→4). See LOG + D-022.
- **MOM-159** Fix: ATS coverage false-missed JD keywords at sentence ends · **DONE** 2026-07-12 (no schema, bug fix). The JD tokenizer's capture class allows a dot (so "Node.js" survives), which also **swallowed the sentence-final period**: "…experience with Kubernetes." captured `Kubernetes.` and only lowercased it → `kubernetes.`. But the résumé have-set strips trailing dots (`kubernetes`), so the two never matched — a skill the résumé clearly listed was reported **missing**, deflating `coveragePct` and generating a bogus "Add to résumé: Kubernetes." study task (MOM-134 generate-tasks). Extremely common: any skill at the end of a clause ("proficiency in Go.", "must have Kubernetes."). Fix strips trailing dots from JD tokens so both sides normalize identically (and, as a bonus, stops sentence-final stop-words like "We." from slipping past the stop list). Counterfactual unit test (fails on the old code, passes on the fix); the buggy path is pure string logic with no DB/network, so verification is the counterfactual test rather than a live round-trip. 325 API tests.
- **MOM-158** Fix: "am I ready?" verdict collapsed to a false 0/100 on a disjoint company focus · **DONE** 2026-07-12 (no schema, bug fix). `companyWeightedPercentage` divided by the sum of focus weights *that matched a role-track area*. A company's `focusAreas` (MOM-121) is validated against `CAREER_ROLE_AREA_IDS` but **not** against the chosen role track's checklist areas, so the two can be disjoint — and then `weightSum` was 0 and the function returned a hard **0**, reporting a genuinely strong candidate as "0/100, not ready" for `getJobReadiness`, and burying the company at the bottom of `getTargetShortlist`. Now returns `null` for "no applicable weights" and both callers fall back to the flat overall percentage; the `weakestAreas` sort likewise falls back to lowest-percentage instead of an all-zero `focusDrag` no-op. Not reachable on pristine seed (every role track shares `dsa` with every seed company's focus) — it is a latent bug for **user-created** catalog data. Counterfactual-verified unit test (fails on the old `return 0`, passes on the fix) + live: NVIDIA job with focus forced to `{behavioral:5}` (disjoint from its `hpc-gpu-engineer` track) scored 34 (flat) not 0, restored to baseline 50 after. 324 API tests.
- **MOM-157** Résumé drift enters the Today loop · **DONE** 2026-07-12 (no schema). MOM-155 made drift visible *on the résumé page* — the one page you don't open unless you already suspect the résumé is behind. Drift only helps **before** you send, so it belongs on Today. `recommendations.service` now surfaces the single most-drifted version (via `ResumesService.driftSummary` — one profile read + one versions read, no N+1): **priority 74** (between deadlines 90 and stalls 68) when that version is linked to a job still in `saved` — fixable before you apply — with copy naming the company ("Refresh your … résumé before applying to NVIDIA"); **priority 45** (a quiet background reminder, below routine study) otherwise. Only the top-drifted version surfaces, never a pile. 323 API tests. Live: a version linked to the saved NVIDIA job, with CUDA/OpenGL/GPU-Ray-Tracer added to the profile after it was cut, produced `missingCount:3` via the live drift path and hit the pre-send branch; the card is priority-74 so the saturated demo account (interview in 3 days + repair signals) correctly outranks it in the top-8 — card construction is exhaustively unit-tested. Profile restored afterwards.
- **MOM-156** Web tests for `/profile/resumes` · **DONE** 2026-07-12 (no schema). Both bugs fixed today lived in the **web** layer, and the page — by now the densest in the app (editor, ATS, AI critique, rewrites, cover letter, drift, conversion) — had **zero tests**; the API suite could not have caught either. Six tests pin: stored rewrites restored on load (the MOM-154 bug), accept → draft patched + suggestion retired + Save writes text *and* pruned list in one call, accept-on-an-edited-away-bullet → refused and kept (MOM-153's user-side half), reject → persisted, drift panel → names what is missing / silent when not stale. **They immediately caught a flaw in MOM-154's own UI**: the "Applied to the draft" confirmation was rendered *inside* the suggestions list, which unmounts when the last suggestion is accepted — so it was invisible precisely when the user most needed it. 32 web tests.
- **MOM-155** Résumé drift — `baseProfileSnapshot` finally answers the question it was stored for · **DONE** 2026-07-12 (no schema). Found by the write-only-column sweep MOM-154 suggested: `ResumeVersion.baseProfileSnapshot` has recorded, since MOM-132, exactly what the `Profile` looked like when a version was cut — and **nothing ever read it** (not the API, not even the response type). So a résumé rotted silently: you ship a project, and the version you keep sending still predates it, with no signal anywhere. `GET /resumes/:id/drift` diffs the profile against that snapshot **by identity** (skill string / project name / company+role, case-insensitive) rather than deep equality — an edited description is not worth nagging about, a missing project is. A hand-written version has no provenance, so it says `hasSnapshot:false` instead of guessing. `/profile/resumes` shows an amber "Your profile has moved on since this version was cut" panel naming exactly what is missing. Live: version cut → clean; added CUDA + a GPU Ray Tracer project + an NVIDIA role to the profile → `isStale:true` naming all three; hand-written version → `hasSnapshot:false`. (The demo profile was restored to its exact prior state afterwards.) 319 API tests.
- **MOM-154** `aiSuggestions` was a write-only column · **DONE** 2026-07-12 (no schema). MOM-137 persisted rewrites to `ResumeVersion.aiSuggestions` "so the UI can accept/reject them across sessions" and the API serialized them — but **the web page never read them**: a reload silently threw your suggestions away while they sat invisible in the DB. The same dead-end shape as the MOM-146 Readwise ledger. Outstanding rewrites are now restored with the résumé, and accepting/rejecting one *retires* it: accept applies the text and prunes the list in **one** `PATCH` (so an applied rewrite cannot come back from the dead), reject persists immediately without touching `contentMd` (so an unsaved draft edit survives). `UpdateResumeVersionDto` validates each rewrite (nested, max 50) rather than trusting an opaque Json blob, and the serializer filters malformed rows so the response type's `ResumeBulletRewrite[]` promise is worth something. Also closed the *user-side* half of MOM-153's silent-no-op bug: accepting a rewrite whose bullet has since been edited out of the draft now says so and keeps the suggestion, instead of retiring a button press that changed nothing. Live: 2 rewrites survived a reload (both verbatim-applicable), accept → text applied + list pruned + no resurrection on reload, reject → gone, malformed row → 400. 314 API tests.
- **MOM-153** Rewrite + cover letter reach parity with the analysis — and "Accept" stops lying · **DONE** 2026-07-12 (no schema). MOM-149/150 gave *analyze* a target and evidence; **rewrite and cover-letter were left behind**, still taking only a hand-pasted `jdText` and seeing neither the company, its focus areas, the profile, nor its sponsorship posture — a strictly weaker brief than the critique of the very same bullets. All three now share one `TailorContext` and one loader: `jdText` is optional, falling back to the JD already stored on the targeted (or linked) application, so **a JD captured once in the pipeline is never pasted twice**; no JD anywhere → `{ok:false}` with no model call and nothing billed (live: `ai/usage` unmoved). The cover letter's visa paragraph is now calibrated to `Company.sponsorshipStatus ?? JobApplication.visaTag` — three distinct framings, and the `not_sponsoring` case is told *not* to paper over it. **Bug found and fixed:** the UI applies a rewrite with `contentMd.replace(original, rewritten)`, but models return the *normalized* bullet we showed them, which does not match a PDF-extracted résumé's double spaces — so **"Accept" silently did nothing**. Rewrites are now snapped back to the verbatim source line (unmatchable ones dropped rather than offered as a dead button). Live on a deliberately double-spaced bullet: body `{}` → JD pulled from the linked NVIDIA app, `original` returned verbatim, `in-source: True` (the normalized form: `False`). 310 API tests.
- **MOM-151/152** Résumé findings → actions, and measured · **DONE** 2026-07-11 (no schema). **151**: `POST /resumes/:id/ai/themes-to-tasks` turns the AI's missing themes into study tasks — reuses the MOM-135 gap→task shape, capped at 8, deduped by title, **no model call** (live: created 2 → 0 on repeat, `ai/usage` unmoved). Titles share the `Résumé gap:` prefix with profile-score gaps *deliberately*, so both dedupe against each other. **152**: each version on `/profile/resumes` now shows its real record (`sent · interviewing · offers · conversion%`) from MOM-145's funnel — closing the loop analyse → rewrite → send → measure, the only feedback in the feature that is not an opinion. 302 API tests.
- **MOM-148/149/150** Complete + targeted + evidence-grounded résumé analysis · **DONE** 2026-07-11 (no schema). **148**: the completeness contract is now *checked* — a short answer triggers one repair pass for the missing indexes, merged and billed (both calls recorded); a failed repair keeps the partial. **149**: `analyze` takes an optional `jobApplicationId` and otherwise falls back to `ResumeVersion.jobApplicationId`, so the critique is judged against a real JD + the company's focus areas by default. **150**: the `Profile` is fed in as evidence so suggestions name a metric the candidate already claims instead of emitting a placeholder. Live: the same CV went from "exceptionally strong" (generic) to "severely lacking applied C++, CUDA, and hardware-parallelism" when aimed at the real NVIDIA GPU JD, missingThemes CUDA/GPU-architecture, 14/14 coverage in 36s. 301 API tests.
- **MOM-147** Model-agnostic résumé analysis (completeness contract) + gateway pricing · **DONE** 2026-07-11 (no schema). Measured 6 models on a real 14-bullet CV: the cheap tiers are cheap partly because they **silently critique fewer bullets** — Gemini 3.1 Pro covered **3/14** while looking 3× cheaper than Opus (per bullet audited it was *dearer* than Opus); Gemini 3 Flash self-contradicted (score 5 + `issue: "None. This is a perfect bullet"`). Root cause was ours: nothing required completeness. Fixed by extracting bullets **in code** (`ResumeAiService.extractBullets`), enumerating them, adding an `index` field so coverage is checkable, and telling the model to ignore PDF whitespace artifacts; `MAX_TOKENS` 4096→16000 (adaptive thinking bills inside it). **Gemini 3.1 Pro 3/14 → 14/14**, $0.041/audit vs Opus $0.116, ~2× faster — now the default. Gateway pricing rows added (the opus fallback under-reported a Fable-tier model by 2×, which could overspend the daily cap). 296 API tests; live: 14/14 indexes 0–13 in 26s, billed $0.0404. *Caveat: the Gemini slug is a **preview** and Haiku 4.5 failed schema parsing 2 of 3 runs (intermittent, unexplained).*
- **MOM-145** Conversion analytics by source **and** résumé version · **DONE** 2026-07-10 — `jobs.service.funnel()` gains `byResumeVersion`, joined through `ResumeVersion.jobApplicationId` ("this is what I sent them") and keyed by version label; the generic `breakdown()` helper now drops null keys, so applications with **no** version linked are excluded rather than diluting the comparison (the row stays hidden until versions are linked). Funnel card renders sent / interviewing / offers / conversion per version. 293 API tests; live: a "Big-tech tailored v2" version linked to the Databricks (offer) + Meta (interview) apps → 2 sent, 2 interviewing, 1 offer, 50% conversion, vs "Base résumé" on the rejected Airbnb app → 0%; the other 5 apps correctly uncounted.
...
| SPIKE-014 | **RESOLVED** 2026-07-10 (ADR-0012 ACCEPTED, D-020). `ResumeVersion` decoupled from `@unique Profile`; `contentMd` = Markdown derived from Profile via `profileToMarkdown` (deterministic-first). PDF-export library choice deferred to MOM-139; AI tailoring budget reuses `grading.service`/MOM-068 pattern (MOM-136–138). | MOM-132, MOM-136 |
| SPIKE-015 | ✅ RESOLVED (MOM-114, ADR-0015): minimal `Offer` (base/bonus/equityTotal Decimal + equityYears), normalized annual total at read time, **single-currency v1 — no FX** (mixed-currency warning in the UI). Vesting cliffs / tax / COL deferred to v2. | MOM-114 |
